...
def get_local_data_files():
    supported_exts = (".nii.gz", ".json", ".tsv", ".edf", ".snirf", ".nirs")
    local_files = []
...
modality = st.sidebar.selectbox("Select Modality", ["fMRI (.nii.gz)", "EEG (.edf)", "fNIRS (.snirf / .nirs)"])

snirf_files = [f for f in local_data_files if f.endswith(".snirf") or f.endswith(".nirs")]
...
if modality == "fMRI (.nii.gz)":
    ...
    selected_json = st.sidebar.selectbox("Select accompanying .json file", json_files if json_files else ["No JSON files found"])
elif modality == "fNIRS (.snirf / .nirs)":
    selected_snirf = st.sidebar.selectbox("Select fNIRS .snirf/.nirs file", snirf_files if snirf_files else ["No fNIRS files found"])
    selected_json = st.sidebar.selectbox("Select accompanying .json file", json_files if json_files else ["No JSON files found"])
...
elif modality == "fNIRS (.snirf / .nirs)" and selected_snirf and selected_json and (selected_snirf.endswith(".snirf") or selected_snirf.endswith(".nirs")):
    st.success("✅ Selected fNIRS data file and metadata")

    try:
        raw = mne.io.read_raw_snirf(selected_snirf, preload=True)
        st.subheader("🧠 fNIRS SNIRF Information")
        st.text(raw.info)
        st.write("Channels:", raw.ch_names)
    except Exception as e:
        st.error(f"Failed to load fNIRS data: {e}")
...
if modality == "fMRI (.nii.gz)":
    ...
    from analysis_and_visualisation.fnirs_plot import plot_multiple_fnirs_channels
    actions = [
        "Plot fNIRS Channels",
        "Plot Oxygenation Levels (Coming Soon)"
    ]
    selected_action = st.selectbox("Select an analysis to perform:", actions)

    if st.button("Run Selected Analysis"):
        if selected_action == "Plot fNIRS Channels":
            plot_multiple_fnirs_channels(raw)
        else:
            st.info("This action is not yet implemented.")
